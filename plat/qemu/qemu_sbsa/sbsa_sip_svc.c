			       ...
			       u_register_t flags)
{
	...
	case SIP_SVC_VERSION:
		...
		SMC_RET3(handle, NULL, sbsa_platform_dt_version_major(),
			 sbsa_platform_dt_version_minor());

	case SIP_SVC_GET_GIC:
		SMC_RET3(handle, NULL, sbsa_get_gicd(), sbsa_get_gicr());

	case SIP_SVC_GET_GIC_ITS:
		SMC_RET2(handle, NULL, sbsa_platform_dt_gic_its_addr());

	case SIP_SVC_GET_CPU_COUNT:
		SMC_RET2(handle, NULL, sbsa_platform_dt_num_cpus());

	case SIP_SVC_GET_CPU_NODE:
		index = x1;
		if (index < PLATFORM_CORE_COUNT) {
			cpu_data data = sbsa_platform_dt_cpu_node(index);
	...
	case SIP_SVC_GET_CPU_TOPOLOGY:
		cpu_topology topology = sbsa_platform_dt_cpu_topology();
		if (topology.cores > 0) {
	...
	case SIP_SVC_GET_MEMORY_NODE_COUNT:
		SMC_RET2(handle, NULL, sbsa_platform_dt_num_memnodes());

	case SIP_SVC_GET_MEMORY_NODE:
		index = x1;
		if (index < PLAT_MAX_MEM_NODES) {
			memory_data data = sbsa_platform_dt_memory_node(index);
