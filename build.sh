...
if [ "$1" == "fvp" ]; then
   ...
   make CROSS_COMPILE=aarch64-none-elf- PLAT=fvp DEBUG=0 LOG_LEVEL=40     \
        ARM_DISABLE_TRUSTED_WDOG=1 ARM_ARCH_MAJOR=9 ARM_ARCH_MINOR=2      \
        BRANCH_PROTECTION=1 CTX_INCLUDE_AARCH32_REGS=0 ENABLE_FEAT_RME=1  \
        ENABLE_RMM=1 RMM_V1_COMPAT=0                                      \
        FVP_HW_CONFIG_DTS=fdts/fvp-base-gicv3-psci-1t.dts                 \
   ...
   make CROSS_COMPILE=aarch64-none-elf- PLAT=qemu DEBUG=1 LOG_LEVEL=40    \
        ENABLE_FEAT_RME=1 ENABLE_RMM=1 RMM_V1_COMPAT=0                    \
